...
반영되도록 고침 — 1차 수정 후에도 "나이가 잘 안 반영된다"는 피드백이 또 나와서, `age`를
문장 뒤쪽에 부가절로 붙이는 대신(`... who looks approximately N years old`, 다른 지시에
묻혀서 잘 안 챙겨졌음) **나이대+성별을 프롬프트 맨 앞 주어로 압축**하는 방식으로 바꿈
(`describeAgeGender()` — 예: "a female model in their mid-20s (around 25 years old)").

**이미지 퀄리티 저하 이슈**: "너무 인위적이다"는 피드백에 대응해 넣었던 촬영기법 지시(필름
그레인/모션 블러/살짝 초점 흐림)가 오히려 "저화질처럼 보인다"는 새 피드백으로 이어짐 — 자연스러운
포즈는 유지하되 그레인·블러 지시는 빼고 "sharp, high-resolution, avoid grainy/blurry" 같은
선명함 요구를 명시적으로 추가함. 단, 이건 **프롬프트로 조절 가능한 스타일 문제**이고, `member`
티어 이미지가 `low` quality 파라미터로 생성되는 건 원가 절감을 위한 **의도된 설계**라 별개임
(위 "유저 티어 구조" 표 참고) — 사용자가 "그래도 여전히 퀄리티가 아쉽다"고 하면 이 quality
파라미터 자체를 올릴지(원가 상승 트레이드오프) 확인이 필요함.
