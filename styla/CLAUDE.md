...
| 티어 | 한도 | 입력 | 결과 | 이미지 |
|---|---|---|---|---|
| 게스트 | **평생 1회** (localStorage 영구 플래그, 날짜 리셋 아님) | 성별/나이/키/몸무게(성별·키·몸무게 필수) + "상세조건 더보기": 가슴·허리·엉덩이둘레/다리길이/계절·날씨/TPO/선호 무드·스타일 | title/keywords/paragraphs 블로그·매거진 스타일 리포트 (로그인과 동일) | 1장 (low quality) |
...
- **게스트와 로그인(member)의 차이는 이용 한도(평생 1회 vs 하루 3회)뿐**, 출력 스키마/이미지 화질은 완전히
  동일함(2단계 개편). 원래는 게스트만 JSON 없이 2~3문장 텍스트만 주는 별도 경로였는데, "게스트도 로그인과
  출력 내용 똑같이" 요청으로 `generate-recommendation`의 `buildTextPrompt`/`buildImagePrompts`에서
  guest/member 분기를 합쳤고, `ResultView.jsx`도 `GuestResult`를 없애고 `MemberResult`를 공유하도록
  정리함(단, 저장 버튼은 게스트에게 `onToggleSave`를 안 넘겨서 계속 숨김 — 게스트는 계정이 없어 저장 불가).
