...
회원탈퇴, usage_logs 보안 강화, 이미지 1회=1장 개편(1-1) — 전부 완료 및 브랜치에 푸시됨.

## 진행 중 / 다음 할 일 (Styla+ 개선 우선순위, 사용자가 준 순서)

- [x] 🔴 1-1: 이미지 1회=1장 전환, 화질 조정, 로그인 티어 블로그 리포트화 — **완료**
- [ ] 🟡 2-1: 초기 입력 폼 간소화 — 무료/게스트는 성별+체형카드(표준/마른/통통/근육형)+컬러톤+TPO만, 정밀 치수(가슴/허리/엉덩이둘레·다리길이·몸무게)는 프리미엄 진입 시점으로 이동
