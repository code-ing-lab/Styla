...
(`claude/prompt-generation-5849zj`)로 고정돼 있어서 프론트엔드 변경사항이 실제 사이트에 반영이
안 되고 있던 것도 발견해서, 사용자가 `claude/claude-md-reading-nn8tx0`로 직접 바꿔서 해결함
(Netlify 대시보드 → Project configuration → Build & deploy → Continuous deployment →
Production branch).

## 다음 할 일 (Styla+ 개선 우선순위)

- [ ] 🔴 **Edge Function 재배포** — 이미지 프롬프트를 새로 짠 뒤로 아직 배포 안 됐을 가능성 높음.
  `npm run functions:deploy`로 반영 후, 로그인/프리미엄 각각 한 번씩 추천 받아서 이미지가
  예전보다 자연스러워졌는지(스톡사진 느낌 vs 캔디드 스냅 느낌) 눈으로 비교할 것. 텍스트 리포트도
  스키마대로 잘 나오는지(빈 필드 없는지), 부적절한 표현이 섞이진 않는지 같이 확인.
  - 디버깅 팁: 만약 화면에 `bodyType.primary`나 리포트 문장이 비어 보이면, 십중팔구 "로컬은
    최신인데 Edge Function 배포가 안 됐거나 예전 버전"인 경우다 — `git pull`로 로컬이 최신인지
    먼저 확인하고, 확실히 하려면 그냥 한 번 더 `functions:deploy`.
...
- ~~`OPENAI_API_KEY` secret 등록~~ — **완료** (사용자가 OpenAI에서 키 발급 후 등록, 크레딧 충전도 완료).
- **이미지 프롬프트 재작성분 Edge Function 재배포** — 위 "다음 할 일" 맨 위 항목과 동일, 아직 안
  됐을 가능성 높음.
- 새 이미지 프롬프트(캔디드 스냅 스타일) 실제 결과물이 기대만큼 자연스럽게 나오는지 — 아직 미확인.
- Netlify가 `claude/claude-md-reading-nn8tx0`를 Production branch로 보고 실제 재배포에 성공했는지 —
  사용자가 설정은 바꿨다고 확인함, 배포 성공 여부까지는 미확인.
