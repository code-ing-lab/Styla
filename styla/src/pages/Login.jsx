    ...
    <div className="mx-auto flex max-w-sm flex-col items-center px-6 py-24 text-center">
      ...
      {mode === 'forgot' ? (
        resetSent ? (
          <p className="mt-8 text-sm text-cream-text dark:text-night-text">
            <strong>{email}</strong>로 비밀번호 재설정 링크를 보냈어요. 메일함(스팸함 포함)을 확인해주세요.
          </p>
        ) : (
          <form onSubmit={handleResetPassword} className="mt-6 w-full text-left">
        ...
        <form onSubmit={handleSubmit} className="mt-6 w-full text-left">
          ...
          {mode === 'login' && (
            <button
              ...
              className="mt-2 text-xs text-cream-subtext underline underline-offset-4 hover:text-cream-text dark:text-night-text/60 dark:hover:text-night-text"
            >
              비밀번호를 잊으셨나요?
            </button>
          )}
      ...
      {mode === 'forgot' && (
        <button
          type="button"
          onClick={() => switchMode('login')}
          className="mt-4 text-xs text-cream-subtext underline underline-offset-4 hover:text-cream-text dark:text-night-text/60 dark:hover:text-night-text"
        >
          로그인으로 돌아가기
        </button>
      )}
